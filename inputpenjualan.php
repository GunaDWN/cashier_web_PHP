...
/* jika ada data yang tidak ditemukan (jumlah data produk 
tidak sama dengan data keranjang)*/
if (mysqli_num_rows($ambil_hasil) != count($semua_produk_id)) :
    // hapus produk di keranjang yang tidak ditemukan
    $semua_produk_tidak_ditemukan = array_diff($semua_produk_id, 
	array_column($array_produk, 'ProdukId'));
    foreach($semua_produk_tidak_ditemukan as 
	$id_produk_tidak_ditemukan){
...
/* loop semua data keranjang, $_SESSION['keranjang']
[$produk_id] = $jumlah*/
foreach($keranjang as $produk_id => $jumlah) :
    // filter/saring produk berdasarkan id
    $filter_produk = array_filter($array_produk, 
	function($produknya_array) use ($produk_id){
      return $produknya_array['ProdukId'] == $produk_id;
    });
    /* jika tidak ada produk yang ditemukan, lompati 
	data ini dan lanjut data berikutnya*/
...
document.location = "homekasir.php?pg=formpenjualan";
    ...
    /* tambah persiapkan data detail untuk dimasukkan 
	ke database nantinya */
...
/* buat transaksi database (dalam cakupan transaksi, 
jika ditemukan gagal pada salah satu query maka query 
lainnya dalam cakupan transaksi bisa dibatalkan 
(kecuali query tertentu))*/
/* https://medium.com/gits-apps-insight/mengenal-konsep-
database-transaction-bagian-1-54e66789f75e*/
mysqli_begin_transaction($koneksi);
// tangkap semua error throwable
try {
    $tanggal_penjualan = date('Y-m-d'); 
	/*Y = 4 digit tahun, m = 2 digit bulan, 
	d = 2 digit hari. ex: 2024-03-24*/
    $persiapan_query = mysqli_prepare($koneksi,
        "INSERT INTO penjualan(PelangganId, 
		TanggalPenjualan, TotalHarga, Pembayaran) 
		VALUES(?, ?, ?, ?)"
    );
    mysqli_stmt_bind_param($persiapan_query,"isdd",
	$pelanggan_id,$tanggal_penjualan,$total_harga, 
	$pembayaran);
    $eksekusi_query = mysqli_stmt_execute($persiapan_query);
    if( ! $eksekusi_query){
    ...
    foreach($persiapan_data_detail as $index => $data_detail){
        ...
        $persiapan_query = mysqli_prepare(
            $koneksi,
            "INSERT INTO detailpenjualan(PenjualanId, ProdukId, 
			JumlahProduk, SubTotal) VALUES(?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($persiapan_query,"iiid",
		$id_penjualan_baru,$data_detail['ProdukId'],
		$data_detail['JumlahProduk'], $data_detail['SubTotal']);
        $eksekusi_query = mysqli_stmt_execute($persiapan_query);
        if( ! $eksekusi_query){
            throw new Exception($message = "Gagal menambah 
			detail penjualan ke " . ($index + 1));
        }
        // kurangi stok produk
        $persiapan_query = mysqli_prepare(
            $koneksi,
        ...
        mysqli_stmt_bind_param($persiapan_query,"ii",
		$data_detail['JumlahProduk'],$data_detail['ProdukId'],);
        $eksekusi_query = mysqli_stmt_execute($persiapan_query);
        if( ! $eksekusi_query){
        throw new Exception($message = "Gagal menambah 
		detail penjualan ke " . ($index + 1));
